import {
  ...
  NO_DAMAGE_ENGAGE_TIME,HOLD_DEBOUNCE,HELP,
} from './constants.js';
...
let healEffect={active:false,timer:0,amount:0,x:0,y:0}; // heal visual effect
...
function startFight(){
  ...
  p2.moveSpeed=AI_LEVELS[currentFight].moveSpeed;
  currentMaxHP=LEVEL_HP[currentFight]||MAX_HP;
  p1.specialCharge=0;p2.specialCharge=0;p1.resetStats();startRound();
}
...
function startRound(){
  ...
  helpUsed=false;helpAvailable=false;
  aiSpecialUsesThisRound=0;
  ai.thinkTimer=0;ai.noDamageForced=false;projectiles.length=0;
  bgmPause();
...
function checkVLS(){
  ...
  if(gap>VLS.hpGapThreshold){
    if(!p1.specialActive&&!p2.specialActive){
      const p1SpecDur=p1.specialName==='ABALONE MOTIVATION'?SPECIAL.playerDuration:SPECIAL.aiDuration;
      const p2SpecDur=p2.specialName==='ABALONE MOTIVATION'?SPECIAL.playerDuration:SPECIAL.aiDuration;
...
function checkDashKick(atk,def){
  ...
  if(dist<35){
    ...
    def.applyHit(fakeData,fromRight,atk.specialActive);
    atk.addSpecialCharge(dmg);
    if(atk.specialActive&&atk.specialName==='REDBULL RECHARGE'){atk.hp=Math.min(atk.hp+Math.round(dmg*SPECIAL.redbullLifesteal),currentMaxHP);}
...
function updateFighting(){
  ...
  // Help trigger: both A+B pressed simultaneously
  ...
  switch(gameState){
    ...
    case 'helpChoice':
      ...
      if(justPressed('attackB')){
        ...
        healEffect={active:true,timer:HELP.healEffectDuration,amount:healAmt,x:p1.x,y:p1.y};
